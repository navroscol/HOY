...
from app.services.checklists import required_documents
...
def create_shipment(
    ...
    for doc_type in required_documents(shipment.vertical, shipment.mode):
        session.add(Document(shipment_id=shipment.id, doc_type=doc_type, status="missing"))
    session.commit()
    return shipment
...
def update_status(shipment_id: int, payload: ShipmentStatusUpdate, org_id: int = Depends(org_id_header), session: Session = Depends(get_session)):
    ...
    if not shipment or shipment.org_id != org_id:
        raise HTTPException(status_code=404, detail="shipment not found")
    old = shipment.status
    if not can_transition(old, payload.to_status):
        raise HTTPException(status_code=422, detail=f"invalid transition {old} -> {payload.to_status}")
...
def list_tasks(
    ...
    shipments = session.exec(select(Shipment.id).where(Shipment.org_id == org_id)).all()
    if not shipments:
        return []
    q = select(Task).where(Task.shipment_id.in_(shipments))
    if status:
...
def ops_today(org_id: int = Depends(org_id_header), session: Session = Depends(get_session)):
    shipments = session.exec(select(Shipment).where(Shipment.org_id == org_id)).all()
    at_risk = [s for s in shipments if s.risk_score >= 60]
    shipment_ids = [s.id for s in shipments]
    tasks = session.exec(select(Task).where(Task.shipment_id.in_(shipment_ids), Task.status != "done")).all() if shipment_ids else []
    open_incidents = session.exec(select(Incident).where(Incident.shipment_id.in_(shipment_ids), Incident.status != "closed")).all() if shipment_ids else []
    return {
        "at_risk": at_risk,
        "open_tasks": tasks,
